  ...
  <div class="max-w-3xl mx-auto p-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow">
      ...
      <form action="{{ route('formacion.update', $formacion->id) }}" method="POST" class="space-y-4">
        ...
        @method('PUT')
        <div>
          <label for="nivel" class="block text-sm font-medium text-gray-700">Descripción</label>
          <input type="text" name="nivel" id="nivel" value="{{ old('nivel',$formacion->nivel) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
        </div>

        <div>
          <label for="titulo_obtenido" class="block text-sm font-medium text-gray-700">Categoría</label>
